...
type taskProduceService struct{}

// AutoRemindNotEnd 今天未结束
func (s *taskProduceService) AutoRemindNotEnd(ctx context.Context) {
	// dutyMsgData 责任人
	dutyMsgData := make(map[string]map[string][]string, 0)
	// joinMsgData 关联责任人
	joinMsgData := make(map[string]map[string][]string, 0)
	resData, _ := dao.ProductTaskProduce.GetAll(ctx, &entity.ProductTaskProduce{
		TaskStatus:  2,
...
func (s *taskProduceService) AutoRemindNotStart() {
...
// AutoUpgrade 升级提醒
func (s *taskProduceService) AutoUpgrade() {
	ctx := context.Background()
	// threeMsgData 第三次升级时提醒人
	threeSendUser, _ := s.autoUpgradeSearch(ctx, &entity.ProductTaskProduce{
		TaskStatus:       2,
		UpgradeThreeTime: gtime.Now(),
	}, "第三次", []string{})

	// twoMsgData 第二次升级时提醒人
	twoSendUser, _ := s.autoUpgradeSearch(ctx, &entity.ProductTaskProduce{
		TaskStatus:     2,
	...
	s.autoUpgradeSearch(ctx, &entity.ProductTaskProduce{
		TaskStatus:       2,
		UpgradeFirstTime: gtime.Now(),
	}, "第一次", twoSendUser)
}

func (s *taskProduceService) autoUpgradeSearch(ctx context.Context, where *entity.ProductTaskProduce, keyStr string, notSendList []string) (sendUserList []string, err error) {
	// msgData 升级时提醒人
	msgData := make(map[string]map[string][]string, 0)
	sendUserList = make([]string, 0)
	// 未完成升级
	upData, err := dao.ProductTaskProduce.GetUpgradeAll(ctx, where)
...
// GetProduceList 获取员工信息列表
func (s *taskProduceService) GetProduceList(ctx context.Context, input *model.ProductTaskWhere) (apiRes *response.GetListResponse, err error) {
	apiRes, err = dao.ProductTaskProduce.GetList(ctx, input)
	if err != nil {
		return apiRes, err
	}
...
// ProduceModify 任务状态变更
func (s *taskProduceService) ProduceModify(ctx context.Context, input *model.ProductTaskWhere) (apiRes *response.GetListResponse, err error) {
	apiRes = &response.GetListResponse{}
	info, err := dao.ProductTaskProduce.GetOne(ctx, input)
	if err != nil {
		return apiRes, err
	}
	...
	apiRes.Data, err = dao.ProductTaskProduce.Modify(ctx, info)
