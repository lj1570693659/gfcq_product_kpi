...
func (s *taskQualityService) AutoRemindNotEnd(ctx context.Context) {
	// dutyMsgData 责任人
	dutyMsgData := make(map[string]map[string][]string, 0)
	// joinMsgData 关联责任人
	joinMsgData := make(map[string]map[string][]string, 0)
	resData, _ := dao.ProductTaskQuality.GetAll(ctx, &entity.ProductTaskQuality{
		TaskStatus:  2,
...
func (s *taskQualityService) AutoRemindNotStart() {
...
// AutoUpgrade 升级提醒
func (s *taskQualityService) AutoUpgrade() {
	fmt.Println("AutoUpgrade------------", gtime.Now().String())
	ctx := context.Background()
	// threeMsgData 第三次升级时提醒人
	threeSendUser, _ := s.autoUpgradeSearch(ctx, &entity.ProductTaskQuality{
		TaskStatus:       2,
		UpgradeThreeTime: gtime.Now(),
	}, "第三次", []string{})

	// twoMsgData 第二次升级时提醒人
	twoSendUser, _ := s.autoUpgradeSearch(ctx, &entity.ProductTaskQuality{
		TaskStatus:     2,
	...
	s.autoUpgradeSearch(ctx, &entity.ProductTaskQuality{
		TaskStatus:       2,
		UpgradeFirstTime: gtime.Now(),
	}, "第一次", twoSendUser)
}

func (s *taskQualityService) autoUpgradeSearch(ctx context.Context, where *entity.ProductTaskQuality, keyStr string, notSendList []string) (sendUserList []string, err error) {
	// msgData 升级时提醒人
	msgData := make(map[string]map[string][]string, 0)
	sendUserList = make([]string, 0)
	// 未完成升级
	upData, err := dao.ProductTaskQuality.GetUpgradeAll(ctx, where)
...
// GetQualityList 获取员工信息列表
func (s *taskQualityService) GetQualityList(ctx context.Context, input *model.ProductTaskWhere) (apiRes *response.GetListResponse, err error) {
	apiRes, err = dao.ProductTaskQuality.GetList(ctx, input)
	if err != nil {
		return apiRes, err
	}
...
// QualityModify 任务状态变更
func (s *taskQualityService) QualityModify(ctx context.Context, input *model.ProductTaskWhere) (apiRes *response.GetListResponse, err error) {
	apiRes = &response.GetListResponse{}
	info, err := dao.ProductTaskQuality.GetOne(ctx, input)
	if err != nil {
		return apiRes, err
	}
	...
	apiRes.Data, err = dao.ProductTaskQuality.Modify(ctx, info)
