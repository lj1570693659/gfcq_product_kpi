...
type taskNpiService struct{}

// AutoRemindNotEnd 今天未结束
func (s *taskNpiService) AutoRemindNotEnd(ctx context.Context) {
	// dutyMsgData 责任人
	dutyMsgData := make(map[string]map[string][]string, 0)
	// joinMsgData 关联责任人
	joinMsgData := make(map[string]map[string][]string, 0)
	resData, _ := dao.ProductTaskNpi.GetAll(ctx, &entity.ProductTaskNpi{
		TaskStatus:  2,
...
func (s *taskNpiService) AutoRemindNotStart() {
...
// AutoUpgrade 升级提醒
func (s *taskNpiService) AutoUpgrade() {
	fmt.Println("AutoUpgrade------------", gtime.Now().String())
	ctx := context.Background()
	// threeMsgData 第三次升级时提醒人
	threeSendUser, _ := s.autoUpgradeSearch(ctx, &entity.ProductTaskNpi{
		TaskStatus:       2,
		UpgradeThreeTime: gtime.Now(),
	}, "第三次", []string{})

	// twoMsgData 第二次升级时提醒人
	twoSendUser, _ := s.autoUpgradeSearch(ctx, &entity.ProductTaskNpi{
		TaskStatus:     2,
	...
	s.autoUpgradeSearch(ctx, &entity.ProductTaskNpi{
		TaskStatus:       2,
		UpgradeFirstTime: gtime.Now(),
	}, "第一次", twoSendUser)
}

func (s *taskNpiService) autoUpgradeSearch(ctx context.Context, where *entity.ProductTaskNpi, keyStr string, notSendList []string) (sendUserList []string, err error) {
	// msgData 升级时提醒人
	msgData := make(map[string]map[string][]string, 0)
	sendUserList = make([]string, 0)
	// 未完成升级
	upData, err := dao.ProductTaskNpi.GetUpgradeAll(ctx, where)
...
// GetNpiList 获取员工信息列表
func (s *taskNpiService) GetNpiList(ctx context.Context, input *model.ProductTaskWhere) (apiRes *response.GetListResponse, err error) {
	apiRes, err = dao.ProductTaskNpi.GetList(ctx, input)
	if err != nil {
		return apiRes, err
	}
...
// NpiModify 任务状态变更
func (s *taskNpiService) NpiModify(ctx context.Context, input *model.ProductTaskWhere) (apiRes *response.GetListResponse, err error) {
	apiRes = &response.GetListResponse{}
	info, err := dao.ProductTaskNpi.GetOne(ctx, input)
	if err != nil {
		return apiRes, err
	}
	...
	apiRes.Data, err = dao.ProductTaskNpi.Modify(ctx, info)
